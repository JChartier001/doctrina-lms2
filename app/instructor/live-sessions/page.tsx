...
export default function InstructorLiveSessionsPage() {
	...
	const startSessionMutation = useMutation(api.liveSessions.start);
	const cancelSessionMutation = useMutation(api.liveSessions.cancel);
	...
	const handleCreateSession = () => {
		...
		createSessionMutation({
			...
			scheduledFor: newSession.scheduledFor.getTime(),
	...
	const handleStartSession = (sessionId: string) => {
		startSessionMutation({ id: sessionId as Id<'liveSessions'> });
									...
									<div className="grid gap-2">
										...
										<Select
											...
											onValueChange={value =>
												setNewSession({
													...newSession,
													maxParticipants: Number.parseInt(value),
												})
											}
										>
											<SelectTrigger>
						...
						upcomingSessions.map(session => (
							<Card key={session._id}>
								...
								<CardFooter className="flex justify-between">
									<Button variant="outline" onClick={() => handleCancelSession(session._id)}>
										Cancel Session
									</Button>
									<Button onClick={() => handleStartSession(session._id)}>Start Session</Button>
								</CardFooter>
							</Card>
						))
					)}
										...
										upcomingSessions
											...
											.map(session => (
												<div key={session._id} className="mb-4 p-4 border rounded-lg">
													...
													<Button size="sm" onClick={() => handleStartSession(session._id)}>
														Start Session
													</Button>
												</div>
											))
