...
export const CourseCard = ({
  ...
  return (
    <Card
      sx={{ minWidth: {xs: 200}, maxWidth: {xs: 300}, height: '100%', cursor: 'pointer' }}
      ...
      cardContent={
        <Stack sx={{ gap: 2 }}>
          <Typography variant="h5" sx={{height:'75px',overflow: 'hidden', textWrap:'nowrap',  textOverflow: "ellipsis"}}>{title}</Typography>
          <Stack direction="row" justifyContent={'space-between'} alignItems={'center'}>
            <Typography variant="body2"> {category}</Typography>
            <Currency value={price} label={''} />
          </Stack>
          <Rating initialRating={rating} count={ratingCount} size={'medium'} readOnly />
