...
test.describe('API Error Handling', () => {
	...
	test('should NOT swallow unexpected errors', async ({ page }) => {
		...
		await page.evaluate(() => window.buggyFunction());
...
test.describe('Network Retry Logic', () => {
	test('should retry on 500 error and succeed', async ({ page }) => {
		...
		const telemetryEvents = await page.evaluate(() => window.__TELEMETRY_EVENTS__ || []);
...
test.describe('Error Telemetry', () => {
	...
	test('should send errors to Sentry with breadcrumbs', async ({ page }) => {
		...
		await page.addInitScript(() => {
			window.Sentry = {
				captureException: (error: Error, context?: any) => {
					window.__SENTRY_EVENTS__ = window.__SENTRY_EVENTS__ || [];
				...
				addBreadcrumb: (breadcrumb: any) => {
					window.__SENTRY_BREADCRUMBS__ = window.__SENTRY_BREADCRUMBS__ || [];
					window.__SENTRY_BREADCRUMBS__.push(breadcrumb);
				},
			};
		});
		...
		const events = await page.evaluate(() => window.__SENTRY_EVENTS__);
		...
		const breadcrumbs = await page.evaluate(() => window.__SENTRY_BREADCRUMBS__);
