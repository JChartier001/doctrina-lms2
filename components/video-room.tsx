...
export function VideoRoom({
			...
			if (localVideoRef.current) {
				try {
					...
					localVideoRef.current.poster =
						userImage || '/placeholder.svg?height=300&width=400';
				} catch (error) {
	...
	const toggleMute = () => {
		...
		toast.success(
			isMuted
				? 'Microphone unmuted. Microphone unmuted'
	...
	const toggleVideo = () => {
		...
		toast.success(
			isVideoOff
				? 'Camera turned on. Camera turned on'
				: 'Camera turned off. Camera turned off'
		);
	};
	...
	const toggleRecording = () => {
		if (isHost) {
			...
			toast.error(
				'Only the host can record the session. Only the host can record the session'
			);
		}
	};
		...
		<div className='flex flex-col h-full'>
			<div className='flex-1 grid grid-cols-1 lg:grid-cols-3 gap-4 mb-4'>
				...
				<div className='bg-card rounded-lg border shadow-sm'>
					<Tabs
						...
						value={activeTab}
						onValueChange={value =>
							setActiveTab(value as 'chat' | 'participants')
						}
					>
			...
			<div className='flex items-center justify-center gap-2 sm:gap-4 p-4 bg-card rounded-lg border shadow-sm'>
				...
				{isHost && (
					...
					>
						<span
							className={`h-3 w-3 rounded-full ${isRecording ? 'bg-red-500' : 'bg-muted-foreground'}`}
						/>
					</Button>
				)}
