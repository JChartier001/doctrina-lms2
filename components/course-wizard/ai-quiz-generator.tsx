...
import { Loader2, Sparkles, Plus, Edit, Trash2 } from 'lucide-react';
...
export function AIQuizGenerator({
	...
	const handleGenerate = async () => {
		if (!content.trim()) {
			toast.error(
				'Content required. Please provide some content to generate questions from.'
			);
			return;
		}
		...
		try {
			...
			toast.success(
				'Questions generated. Successfully generated ${questions.length} questions.'
			);
		} catch (error) {
			toast.error(
				'Generation failed. Failed to generate questions. Please try again.'
			);
		} finally {
	...
	const handleAddAll = () => {
		...
		toast.success(
			'Questions added. Added ${generatedQuestions.length} questions to your quiz.'
		);
				...
				<CardContent className='space-y-4'>
					<div className='space-y-2'>
						...
						<p className='text-xs text-muted-foreground'>
							The AI will analyze this content to create relevant quiz
							questions.
						</p>
					</div>
											...
											{editingQuestion.options.map((option, oIndex) => (
												...
												>
													...
													<Input
														value={option}
														onChange={e =>
															handleUpdateOption(oIndex, e.target.value)
														}
