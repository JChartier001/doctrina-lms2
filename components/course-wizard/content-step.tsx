...
import { FileText, ListChecks, Plus, Sparkles, Trash2, Upload, Video } from 'lucide-react';
import { useRef, useState } from 'react';
import { useForm } from 'react-hook-form';

import type { CourseData, CourseLesson, QuizQuestion } from '@/app/instructor/courses/wizard/page';
...
import { CreateCourseDefaultValues, CreateCourseLessonType } from '@/schema/CourseWizardSchema';
...
export function ContentStep({ courseData, updateCourseData }: ContentStepProps) {
	...
	// Find all lessons
	const _allLessons = courseData.sections.flatMap(section =>
	...
	const updateLessonContent = (content: string) => {
		...
		const updatedSections = courseData.sections.map(section => {
			if (section.id === selectedLesson.id) {
				return {
					...section,
	...
	const addQuizQuestion = () => {
		if (!selectedLesson || !newQuestion.trim() || newOptions.some(opt => !opt.trim())) return;

		const newQuizQuestion: QuizQuestion = {
			id: `question-${nextQuestionIdRef.current++}`,
		...
		<div className="space-y-6">
			<div>
				...
				<p className="text-muted-foreground mb-6">
					Add content to your lessons. You can upload videos, create documents, or create quizzes.
				</p>
			</div>
				...
				<div className="md:col-span-1 space-y-4">
					<Card>
						...
						<CardContent className="p-0">
							<Accordion type="single" collapsible className="w-full" defaultValue={courseData.sections[0]?.id}>
					...
					{selectedLesson ? (
						<Card>
							<CardHeader>
								...
								{/* <p className="text-sm text-muted-foreground">Section: {selectedLesson.sectionTitle}</p> */}
							</CardHeader>
							<CardContent>
									...
									<div className="space-y-4">
										...
										<div className="space-y-2">
											...
											<div className="border-2 border-dashed rounded-lg p-6 flex flex-col items-center justify-center">
												...
												<p className="text-sm font-medium mb-1">Drag and drop your video file, or click to browse</p>
										...
										<Button onClick={() => updateLessonContent(videoUrl)} disabled={!videoUrl.trim()}>
											Save Video
										</Button>
									</div>
								)}
								...
								{selectedLesson.type === 'document' && (
									<div className="space-y-4">
										...
										<Button onClick={() => updateLessonContent(documentContent)} disabled={!documentContent.trim()}>
											Save Document
										</Button>
									</div>
								)}
									...
									<div className="space-y-6">
										<Tabs value={activeTab} onValueChange={setActiveTab}>
											...
											<TabsContent value="ai" className="pt-4">
												<AIQuizGenerator onAddQuestions={addQuizQuestions} existingContent={documentContent || ''} />
											</TabsContent>
										</Tabs>
									</div>
								)}
						...
						<Card className="flex flex-col items-center justify-center p-6 h-full">
							<div className="text-center space-y-2">
								...
								<p className="text-sm text-muted-foreground">Select a lesson from the sidebar to add content.</p>
							</div>
						</Card>
					)}
				</div>
...
export const ContentStepForm = () => {
	const _form = useForm<CreateCourseLessonType>({
		mode: 'onChange',
	...
	return (
		<Stepper defaultValue={2}>
			{steps.map(({ title, description }, index) => (
				<StepperItem key={index} step={index + 1} className="not-last:flex-1 max-md:items-start">
					...
					{index < steps.length && <StepperSeparator className="max-md:mt-3.5 md:mx-4" />}
				</StepperItem>
			))}
		</Stepper>
	);
