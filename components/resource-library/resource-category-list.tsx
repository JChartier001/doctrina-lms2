...
import { getResourceCategories, type ResourceCategory, getCategoryDisplayName } from "@/lib/resource-library-service"
...
export function ResourceCategoryList() {
  ...
  const categoriesWithResources = resourceCategories.filter(({ count }) => count > 0)
        ...
        {categoriesWithResources
          ...
          .map(({ category, count }) => (
            <Link key={category} href={`/resources?category=${category}`}>
              <Button variant="outline" className="w-full h-full min-h-[5rem] justify-start p-4 hover:bg-accent/50">
                <div className="flex flex-col items-start">
                  <span className="text-sm text-muted-foreground mb-2">{count} resources</span>
