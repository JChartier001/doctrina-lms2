...
export function ResourceCard({ resource }: ResourceCardProps) {
	...
	useState(() => {
		const checkFavoriteStatus = async () => {
			const status = await useIsFavorited(
				user?.id as Id<'users'>,
				resource._id
			);
			setFavorited(status.data || false);
		};
		checkFavoriteStatus();
	});
	...
	const handleToggleFavorite = async () => {
		...
		try {
			if (favorited) {
				await useRemoveFromFavorites(user?.id as Id<'users'>, resource._id);
				setFavorited(false);
			} else {
				await useAddToFavorites(user?.id as Id<'users'>, resource._id);
				setFavorited(true);
			}
		} catch (error) {
	...
	return (
		<Card className='overflow-hidden'>
			...
			<CardFooter className='p-4 pt-0 flex justify-between'>
				...
				>
					<Heart
						className={`h-4 w-4 mr-1 ${favorited ? 'fill-current text-red-500' : ''}`}
					/>
