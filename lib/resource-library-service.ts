...
export function useIsFavorited(userId: Id<'users'>, resourceId: Id<'resources'>) {
	const { isEnabled } = useFeatureFlags();
...
// Utility functions for resource display
	...
	},
	{
		...
		description:
			'Standardized protocols for different types of chemical peels, including pre and post-treatment care.',
		type: 'template',
	...
	},
	{
		...
		description:
			'A detailed guide to skincare formulations and ingredients for aesthetic practitioners.',
		type: 'research',
		categories: ['skincare'],
	...
	},
	{
		...
		description:
			'A collection of case studies demonstrating different approaches to facial rejuvenation.',
		type: 'case-study',
	...
	},
	{
		...
		description:
			'A detailed guide to anatomical danger zones for injectable treatments, with emphasis on safety.',
		type: 'video',
...
// User's favorite resources (for fallback)
...
export async function searchResources(
	...
	return mockResources.filter(resource => {
		...
		const matchesRestricted =
			filters?.restricted === undefined ||
			resource.restricted === filters.restricted;
...
export async function getRelatedResources(
	...
	const relatedResources = mockResources
		...
		.map(r => {
			...
			const sharedCategories = r.categories.filter(category =>
				resource.categories.includes(category)
			).length;
			const sharedTags = r.tags.filter(tag =>
...
export async function removeResourceFromFavorites(
	...
	if (index !== -1) {
		...
		if (
			resourceIndex !== -1 &&
			mockResources[resourceIndex].favoriteCount > 0
		) {
			mockResources[resourceIndex].favoriteCount -= 1;
		}
		return true;
...
export async function isResourceFavorited(
	resourceId: string
): Promise<boolean> {
	// Simulate API delay
...
// Convex-based hooks for resource management
export function useResources() {
	const { isEnabled } = useFeatureFlags();

	const convexResources = useQuery(
		api.resources.list,
		isEnabled('convex_resources') ? {} : 'skip'
	);
...
export function useFeaturedResources(limit = 4) {
	...
	// Fallback to mock data
	const featured = mockResources.filter(r => r.featured).slice(0, limit);
	return {
		data: featured,
		isLoading: false,
		error: null,
...
export function useResourceById(id: string) {
	...
	const convexResource = useQuery(
		api.resources.get,
		isEnabled('convex_resources') && id ? { id: id as Id<'resources'> } : 'skip'
	);
...
export function useUserFavorites(userId: string) {
	...
	const convexFavorites = useQuery(
		api.favorites.listForUser,
		isEnabled('convex_favorites') && userId
	...
	// Fallback to mock data
	const favorites = mockResources.filter(resource =>
...
export function useIsResourceFavorited(userId: string, resourceId: string) {
	const { isEnabled } = useFeatureFlags();

	const convexIsFavorited = useQuery(
		api.favorites.isFavorited,
		isEnabled('convex_favorites') && userId && resourceId
			? {
					userId: userId as Id<'users'>,
	...
	// Fallback to mock data
	const isFavorited = userFavorites.includes(resourceId);
	return {
		data: isFavorited,
		isLoading: false,
		error: null,
...
// Mutation hooks for resource operations
