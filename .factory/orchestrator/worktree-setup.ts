...
export async function prepareWorkspace(
	...
	const resolvedMode: 'worktree' | 'clone' | 'branch' =
		typeof mode === 'boolean' ? (mode ? 'worktree' : 'branch') : mode || 'worktree';
...
async function main() {
	...
	const mode =
		modeArg === 'true' ? true : modeArg === 'false' ? false : (modeArg as 'worktree' | 'clone' | 'branch' | undefined);
	...
	try {
		const result = await prepareWorkspace(args.repoRoot, args.baseDir, args.branch, args.key, args.mode);
