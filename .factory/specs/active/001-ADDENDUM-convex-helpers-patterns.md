...
const NotificationsPage = () => {
	const { isPending, isSuccess, data } = useQueryWithStatus(api.notifications.list);
...
const ProfilePage = () => {
	const { status, data, error, isPending, isError } = useQueryWithStatus(api.users.getCurrent);
...
const CoursePage = ({ courseId }: { courseId: string | null }) => {
	const course = useQuery(courseId ? api.courses.get : 'skip', courseId ? { id: courseId } : undefined);
...
const DashboardPage = () => {
	...
	return <Dashboard courses={coursesQuery.data} user={userQuery.data} notifications={notificationsQuery.data} />;
};
```
...
const NotificationsPage = () => {
	const { isPending, isError, error, data } = useQueryWithStatus(api.notifications.list);
...
export const getCourseWithInstructor = query({
	...
	handler: async (ctx, { courseId }) => {
		...
		const instructor = await getOneFromOrThrow(ctx.db, 'users', '_id', course.instructorId);

		// Get all lessons for this course
		const lessons = await getManyFrom(ctx.db, 'lessons', 'courseId', courseId);
...
_This addendum enhances Spec 001 with superior patterns from convex-helpers. All patterns are production-ready and already available in your dependencies._
