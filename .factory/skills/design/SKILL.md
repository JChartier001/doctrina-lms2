...
function Dropdown() {
	...
	return (
		<div role="listbox" tabIndex={0} onKeyDown={handleKeyDown} aria-activedescendant={activeId}>
			{/* Items */}
		</div>
	);
}
		...
		<Dialog.Root>
			...
			<Dialog.Portal>
				...
				>
					...
					<Dialog.Close asChild>
						<button className="absolute top-2 right-2 p-1" aria-label="Close dialog">
							<XIcon aria-hidden="true" />
						</button>
					</Dialog.Close>
				</Dialog.Content>
...
function AccessibleDropdown() {
	return (
		<DropdownMenu.Root>
			...
			<DropdownMenu.Portal>
				<DropdownMenu.Content className="bg-white rounded-md shadow-lg p-1" sideOffset={5}>
...
function AccessibleTabs() {
	return (
		<Tabs.Root defaultValue="tab1">
			<Tabs.List className="flex border-b border-gray-200" aria-label="Manage your account">
				<Tabs.Trigger
					value="tab1"
	...
	return (
		<Accordion.Root type="multiple" className="space-y-2">
			<Accordion.Item value="item-1" className="border rounded-lg">
				<Accordion.Header>
					<Accordion.Trigger className="flex w-full items-center justify-between p-4 font-medium">
						...
						<ChevronDownIcon aria-hidden="true" className="transition-transform data-[state=open]:rotate-180" />
					</Accordion.Trigger>
				</Accordion.Header>
				<Accordion.Content className="px-4 pb-4">
					Accessibility ensures that people with disabilities can perceive, understand, navigate, and interact with
					websites and tools.
				</Accordion.Content>
			</Accordion.Item>
			...
			<Accordion.Item value="item-2" className="border rounded-lg">
				...
				<Accordion.Content className="px-4 pb-4">
					WCAG (Web Content Accessibility Guidelines) provides a standard for making web content accessible to all
		...
		<Toast.Provider swipeDirection="right">
			...
			>
				...
				<div className="flex-1">
					<Toast.Title className="font-semibold">Update available</Toast.Title>
...
export function Button({ variant, size, disabled, className, ...props }: ButtonProps) {
	return <button className={button({ variant, size, disabled, className })} disabled={disabled} {...props} />;
}
```
