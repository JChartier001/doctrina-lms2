...
export const listTasks = query({
	...
	handler: async (ctx, args) => {
		let query = ctx.db.query('tasks').withIndex('by_user', q => q.eq('userId', args.userId));
...
export const searchTasks = query({
	...
	handler: async (ctx, args) => {
		let searchQuery = ctx.db
			.query('tasks')
			.withSearchIndex('search_title', q => q.search('title', args.query).eq('userId', args.userId));
...
for await (const task of ctx.db.query('tasks').withIndex('by_status', q => q.eq('status', 'pending'))) {
	// Process each task
...
export const createTask = mutation({
	...
	handler: async (ctx, args) => {
		...
		await ctx.scheduler.runAfter(60 * 60 * 1000, internal.tasks.reminderCheck, { taskId });
		...
		await ctx.scheduler.runAt(tomorrow.getTime(), internal.tasks.dailyDigest, { userId: args.userId });
...
// Run every 2 hours
crons.interval('delete old tasks', { hours: 2 }, internal.tasks.deleteOldTasks, {});
...
export async function getUserTasks(ctx: QueryCtx | MutationCtx, userId: Id<'users'>): Promise<Array<Doc<'tasks'>>> {
	return await ctx.db
		.query('tasks')
		.withIndex('by_user', q => q.eq('userId', userId))
		.collect();
...
export const createTask = mutation({
	...
	handler: async (ctx, args) => {
		...
		const recentTasks = await ctx.db
			.query('tasks')
			.withIndex('by_user', q => q.eq('userId', args.userId))
			.filter(q => q.gte(q.field('_creationTime'), Date.now() - 60000))
			.collect();
