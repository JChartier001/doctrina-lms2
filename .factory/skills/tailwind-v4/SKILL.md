...
<div class="bg-(--color-primary-500)">Primary background</div>

<div class="text-(--color-foreground)">Foreground text</div>
```
...
<div class="h-100 w-200 mt-15 text-32">Dynamic sizes without brackets</div>

<!-- Grid columns -->
<div class="grid-cols-15">15 column grid</div>

<!-- Still use parentheses for CSS variables -->
<div class="bg-(--my-color) text-(--my-text-color)">CSS variable references</div>
```
...
<div class="bg-[linear-gradient(to_right,red,blue)]">Gradient background</div>

<div class="grid-cols-[200px_1fr_1fr]">Complex grid template</div>
```
...
<div class="@container/sidebar">
	<div class="@lg/sidebar:flex">Responds to sidebar container</div>
</div>
```
...
<div class="@container">
	...
	<div class="@sm:text-lg">
		...
		<div class="@md:text-xl">
			<!-- min-width: 28rem (448px) -->
			<div class="@lg:text-2xl">
				<!-- min-width: 32rem (512px) -->
				<div class="@xl:text-3xl">
					<!-- min-width: 36rem (576px) -->
					<div class="@2xl:text-4xl"><!-- min-width: 42rem (672px) --></div>
				</div>
			</div>
		</div>
	</div>
...
<div class="@container">
	...
	<div class="block @lg:hidden">Visible when container is small</div>

	<!-- Max-width container queries -->
	<div class="@max-md:text-sm">Small text when container is narrow</div>
</div>
```
...
<div class="rotate-x-45">Rotated 45deg on X-axis</div>

<div class="rotate-y-90">Rotated 90deg on Y-axis</div>

<div class="rotate-z-180">Rotated 180deg on Z-axis (same as rotate-180)</div>

<!-- Arbitrary 3D rotations -->
<div class="rotate-x-[30deg] rotate-y-[45deg]">Custom 3D rotation</div>
```
...
```html
<!-- Translate on Z-axis -->
<div class="translate-z-12">Moved 12px forward in 3D space</div>

<div class="translate-z-[-20px]">Moved 20px backward</div>
```
...
```html
<!-- Scale on Z-axis -->
<div class="scale-z-150">Scaled to 150% on Z-axis</div>

<div class="scale-z-50">Scaled to 50% on Z-axis</div>
```
...
<div class="perspective-1000">
  ...
  "
	>
		<div class="bg-white rounded-lg shadow-lg p-6">Hover for 3D effect</div>
	</div>
</div>
```
...
<div class="bg-linear-[to_right,red,blue]">Left to right gradient</div>

<div class="bg-linear-[45deg,#f00,#00f]">45-degree gradient</div>

<div class="bg-linear-[135deg,red_0%,yellow_50%,green_100%]">Multi-stop gradient</div>
```
...
```html
<!-- Radial gradient -->
<div class="bg-radial-[circle,red,blue]">Circle radial gradient</div>

<div class="bg-radial-[ellipse_at_top,red,blue]">Ellipse at top</div>

<div class="bg-radial-[circle_at_50%_50%,red_0%,blue_100%]">Positioned radial gradient</div>
```
...
```html
<!-- Conic gradient -->
<div class="bg-conic-[from_0deg,red,yellow,green,blue,red]">Color wheel</div>

<div class="bg-conic-[from_45deg_at_50%_50%,red,blue]">Conic from 45deg</div>
```
...
<div class="bg-linear-[in_oklch,red,blue]">Smoother gradient using oklch</div>

<div class="bg-linear-[in_oklch_longer_hue,red,blue]">Gradient with longer hue interpolation</div>
```
...
<div class="fade-in">I animate in automatically!</div>
```
...
<div class="not-hover:opacity-50">Faded when not hovered</div>
...
<li class="not-last:mb-4">Margin on all except last</li>
```
...
```html
<!-- Not disabled -->
<button class="not-disabled:hover:bg-blue-600">Only hover when enabled</button>
...
<!-- Combination -->
<div class="group">
	<p class="not-group-hover:text-gray-500">Gray when group not hovered</p>
</div>
```
...
```html
<div class="group">
	<div class="group-hover:bg-blue-500">Changes when parent hovered</div>

	<div class="group-focus:ring-2">Changes when parent focused</div>

	<div class="group-active:scale-95">Changes when parent active</div>
</div>
```
...
<label class="peer-checked:bg-blue-500">Label changes when checkbox checked</label>

<div class="peer-focus:ring-2">Highlights when checkbox focused</div>

<div class="peer-disabled:opacity-50">Faded when checkbox disabled</div>
```
...
<div class="has-[input:focus]:ring-2">Highlights when any input inside is focused</div>

<div class="has-[>img]:grid-cols-2">2 columns only if contains image</div>

<div class="has-[.error]:border-red-500">Red border if contains error</div>
```
...
```html
<!-- Use like any color -->
<div class="bg-blue-500 text-white">Vibrant oklch blue</div>
```
...
<!-- Max-width utilities -->
<div class="text-2xl max-lg:text-xl max-md:text-lg">Larger on bigger screens</div>
```
...
<!-- Active -->
<button class="bg-blue-500 active:bg-blue-700">Press me</button>

<!-- Disabled -->
<button class="bg-blue-500 disabled:bg-gray-400 disabled:cursor-not-allowed">Disabled</button>
...
<div class="overflow-x-auto scrollbar-hide">Hidden scrollbar</div>
...
<button class="focus:outline-none focus-visible:ring-2">Keyboard accessible</button>
```
...
<div class="transform-gpu hover:scale-110 transition-transform">Hardware accelerated</div>
