...
async function checkPermission(user: User, resource: string, action: 'read' | 'write' | 'delete'): Promise<boolean> {
	const permissions = {
...
async function attemptLogin(email: string, password: string) {
	...
	if (attempts?.lockUntil && attempts.lockUntil > new Date()) {
		const minutes = Math.ceil((attempts.lockUntil.getTime() - Date.now()) / 60000);
...
function isValidUrl(url: string): boolean {
	try {
		...
		// Block private IP ranges
		const privateIPRanges = [/^10\./, /^172\.(1[6-9]|2[0-9]|3[0-1])\./, /^192\.168\./, /^169\.254\./];
