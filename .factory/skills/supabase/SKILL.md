...
```typescript
const { data, error } = await supabase.storage.from('avatars').upload(`public/${userId}/avatar.png`, file, {
	cacheControl: '3600',
	upsert: true,
});
```
...
```typescript
const { data, error } = await supabase.storage.from('avatars').download(`public/${userId}/avatar.png`);
```

### ✅ Good: Get Public URL

```typescript
const { data } = supabase.storage.from('avatars').getPublicUrl(`public/${userId}/avatar.png`);
...
```typescript
const { data, error } = await supabase.storage.from('avatars').remove([`public/${userId}/avatar.png`]);
```
...
serve(async req => {
	...
	const { data: posts, error } = await supabaseClient.from('posts').select('*').limit(10);
...
// Fully typed!
const { data } = await supabase.from('posts').select('id, title, user_id').eq('user_id', userId);
```
