...
export const getUserName = query({
	...
	handler: async (ctx, args) => {
		...
		const user: { name: string } | null = await ctx.runQuery(api.users.getUser, { userId: args.userId });
...
// Run every 2 hours
crons.interval('cleanup old messages', { hours: 2 }, internal.crons.cleanupOldMessages, {});
