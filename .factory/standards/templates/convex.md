...
export const badList = query({
	handler: async (ctx, args: any) => {
		// No validation!
		const messages = await ctx.db.query('messages').collect(); // Fetches ALL messages - performance issue!
...
export const cleanupOldMessages = internalMutation({
	...
	handler: async (ctx, args) => {
		...
		await Promise.all(oldMessages.map(msg => ctx.db.delete(msg._id)));
...
const messages = await ctx.db
	.query('messages')
	.withIndex('by_channel_and_time', q => q.eq('channelId', channelId).gt('createdAt', timestamp))
	.collect();
```
