...
const filteredItems = useMemo(() => {
	return items.filter(item => item.status === 'active').sort((a, b) => b.priority - a.priority);
}, [items]);
...
Tabs.Tab = function Tab({ value, children }: { value: string; children: React.ReactNode }) {
	...
	return (
		<button onClick={() => context.setActiveTab(value)} className={context.activeTab === value ? 'active' : ''}>
			{children}
		</button>
	);
};
...
export const UserCard = React.memo(UserCard, (prevProps, nextProps) => prevProps.user.id === nextProps.user.id);
```
